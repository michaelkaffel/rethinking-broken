...
**Component:** `components/Testimonials.tsx` (client component)

11 reviews total: 10 Amazon verified purchase reviews + 1 Patreon member review.

**Behavior:**
- Auto-advances every 6 seconds; pauses on hover
- Prev/next SVG arrow buttons
- Dot indicators (expand on active slide)
- Uses `useState`, `useEffect`, and `useCallback`

**Layout fix:** Container uses a fixed height (`h-[600px] md:h-[520px]`) rather than `min-h-*` to prevent longer reviews (e.g. Tammy's) from pushing the controls row downward. The quote area has `overflow-hidden`. Controls row (prev/next + dots) is a flex row pinned to the bottom of the fixed container.
...
- **From:** `orders@rethinkingbroken.com` via Resend (domain verified; update `RESEND_FROM_EMAIL` in Vercel before launch)
...
- `orders@rethinkingbroken.com` is configured in Cloudflare Email Routing to forward to `owlchrysalismedicine@gmail.com`
- Forwarding is inactive until Owl clicks the Cloudflare destination verification email
...
### Favicons
Generated via favicon.io (PNG → favicon package). Files placed in `app/` directory. Wired into root layout `metadata` export under the `icons` key.
...
- [x] Supabase RLS enabled on `orders` and `download_tokens` tables — harmless defensive measure; architecture uses server-side service role key so warnings are safe to ignore
...
RESEND_FROM_EMAIL=orders@rethinkingbroken.com  # domain verified — update Vercel to this before launch
OWL_NOTIFICATION_EMAIL=owlchrysalismedicine@gmail.com
MY_NOTIFICATION_EMAIL=michaelk.ocm@gmail.com

# Site URL
NEXT_PUBLIC_SITE_URL=http://localhost:3000  # Vercel: set to https://rethinkingbroken.com before launch (DNS on Cloudflare; Vercel cutover pending)
...
- **Resend sandbox** — only delivers to the Resend account email until domain is verified; all other addresses silently dropped. Domain is now verified.
...
- **Testimonials carousel fixed height** — use `h-[600px] md:h-[520px]` (not `min-h-*`) on the container to prevent longer reviews from pushing the controls row down; pair with `overflow-hidden` on the quote area
- **`ADMIN_SECRET` generation** — use `openssl rand -hex 32` (not base64) to avoid special characters (`/`, `=`, `+`) that corrupt during copy-paste into `.env.local`