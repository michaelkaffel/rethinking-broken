...
## Why the 827 MB Audiobook Dictates the Download Architecture
...
### OG image
Stored at public/og-image.png (1200x630px). Wired in root layout.
metadataBase in root layout resolves /og-image.png to https://rethinkingbroken.com/og-image.png.

### noindex pages
/thank-you and all /admin/* routes are excluded from search engines.
app/admin/layout.tsx covers the entire admin section in one file.
...
sanitizeEmail — trims, lowercases, strips control chars, validates format + RFC 5321 max length (254). Returns string or null.
sanitizeName — trims, strips control chars and HTML tags, collapses internal whitespace. Permissive by design — supports accented, hyphenated, and Unicode names. Returns string or null.

Resend integration uses the Resend SDK (resend.contacts.create) — not raw fetch. Already-subscribed contacts are treated as success.
...
3. Generate a short-lived R2 presigned URL (15 min) with ResponseContentDisposition: attachment; filename="Rethinking-Broken-{type}.{ext}" to force download
...
- [x] Stripe secret key rotated Jun 25 2026 after accidental exposure in NEXT_PUBLIC_ env var
...
STRIPE_WEBHOOK_SECRET=whsec_...          # live endpoint secret from Stripe Dashboard
...
# Site URL
NEXT_PUBLIC_SITE_URL=https://rethinkingbroken.com
...
- Resend sandbox — only delivers to the Resend account email until domain is verified; all other addresses silently dropped
...
- metadata exports are silently ignored on client components — Next.js App Router emits no warning; the tags simply don't appear in head. Fix: split into a server wrapper that exports metadata and a client child component that handles interactivity
- Built-in app/sitemap.ts + app/robots.ts replace next-sitemap — no npm package needed
...
- NEXT_PUBLIC_ env vars are exposed to the browser — never put secret keys (sk_live_, sk_test_) in NEXT_PUBLIC_ vars. Only price IDs and publishable keys belong there.
- Stripe secret key rotation — if a secret key is accidentally exposed, rotate it immediately in Stripe Dashboard -> Developers -> API keys. The old key is blocked instantly.